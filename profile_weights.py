...
class Profile:
    ...
    def combine_date_params(self):
        ...
        while curr <= self.end:
            ...
            # print(curr)
            # print(weights['year'][curr.year])
            # print(weights[time_name][(curr.month,curr.day)])
            # print(weights['day_of_week'][curr.weekday()])
            # print(date_wt)

            self.new_date_weights[curr] = date_wt
            curr += timedelta(days=1)
        # re-weight to get proportions
        self.profile['date_wt'] = self.weight_to_prop(self.new_date_weights)
    ...
    def sample_time(self, day_part, is_fraud):
            #print(day_part)
            hour = 18
            ...
            if is_fraud ==0:
                ...
                if day_part == 'Early Morning':
                    hour = random.randrange(5, 8, 1)
                if day_part == 'Late Morning':
                    hour = random.randrange(8, 11, 1)
                if day_part == 'Early Afternoon':
                    hour = random.randrange(11, 14, 1)
                if day_part == 'Late Afternoon':
                    hour = random.randrange(14, 17, 1)
                if day_part == 'Early Night':  
                    hour = random.randrange(17, 22, 1)
                if day_part == 'Late Night':
                    hour = random.randrange(22, 24, 1)
                if day_part == 'Total Night':
                    hour = random.randrange(0, 5, 1)
    ...
    def sample_from(self, is_fraud):
        ...
        for i, num in enumerate(rand_date):
            ...
            #20% of the time choose a new random category
            if random.randrange(100) <= 40:
                chosen_cat = self.closest_rand(self.profile['categories_wt'], np.random.random(1)[0])

            chosen_amt = self.sample_amt(chosen_cat)
            chosen_daypart = self.closest_rand(self.profile['shopping_time'], rand_cat[i])
            #print(chosen_daypart)
